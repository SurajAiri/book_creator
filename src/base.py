...
class BookCreator:
        ...
        for retry in range(self.BOOK_STRUCTURE_RETRY_LIMIT):
            try:
                ...
                if retry == self.BOOK_STRUCTURE_RETRY_LIMIT - 1:
                    ...
                    self.handle_progress(progress_listener,f"Failed to create book structure after {self.BOOK_STRUCTURE_RETRY_LIMIT} attempts.")
                    raise Exception("Failed to create book structure after retries.")
                
                self.handle_progress(progress_listener,f"Retrying book structure generation. Retry Count: {retry + 1}")

    def handle_progress(self, progress_listener, message, progress_data=None, stop_on_flag=True):
        if progress_listener:
            flag = progress_listener(message, progress_data)
            if stop_on_flag and flag:
                exit()
                return True
        return False
    ...
    def create_book(self, book_context, book_genre, book_name, progress_listener=None):
        ...
        self.handle_progress(progress_listener,f"Starting book creation: {book_name}",stop_on_flag=False)
        ...
        if os.path.exists(book_path) and not os.path.exists(book_progress_path):
            self.handle_progress(progress_listener,f"Book '{book_name}' already exists. No further processing is required.",stop_on_flag=False)
            return
        ...
        if not progress_data["intro_generated"]:
            ...
            self.handle_progress(progress_listener,"Successfully saved book introduction.",progress_data)
        ...
        for chapter_index in range(progress_data['chapter_index'], len(data['chapters'])):
            ...
            self.handle_progress(progress_listener,f"Generating content for chapter: {chapter_title}",progress_data)
            ...
            if progress_data['heading_index'] == -1:
                ...
                self.handle_progress(progress_listener,f"Successfully saved chapter intro: {chapter_title}",progress_data)
            ...
            for heading_index in range(progress_data['heading_index'], len(chapter_headings)):
                ...
                self.handle_progress(progress_listener,f"Generating content for heading: {heading_title}",progress_data)
                ...
                if progress_data['subheading_index'] == -1:
                    ...
                    self.handle_progress(progress_listener,f"Successfully saved heading content: {heading_title}",progress_data)
                ...
                for subheading_index in range(progress_data['subheading_index'], len(sub_headings)):
                    ...
                    self.handle_progress(progress_listener,f"Successfully saved subheading content: {sub_heading}",progress_data)
        ...
        if progress_listener:
            self.handle_progress(progress_listener,"Book creation process completed.\nBook saved at: " + book_path,stop_on_flag=False)
